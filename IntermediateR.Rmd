...
__Remark:__ In `R` the `if` statement, as described above, will only accept a __single__ value.       
...
__Remark:__ ``R`` accepts both ``ifelse`` statements structured as outline above, but also ``ifelse`` statements using the built-in ``ifelse()`` function. This function accepts a vector of inputs and is structured as follows: 
\begin{center}
\begin{verbatim}
